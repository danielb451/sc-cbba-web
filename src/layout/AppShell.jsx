...
export default function AppShell() {
  ...
  const systemName = runtime.data?.['system.name'] || 'Seguridad Ciudadana';
  useEffect(() => {
  ...
  const displayName = guard
    ? `${guard.firstName} ${guard.lastName}`
    : user?.username || 'Usuario';
  const detail = guard
    ? `${guard.code} · ${guard.rank || 'Guardia'}`
    : user?.role || '';

  const visible = useMemo(
    () =>
      navItems.filter(
        (item) => item.loose || !item.permission || can(item.permission),
      ),
    [user],
  );
      ...
      <aside className={`sidebar ${open ? 'sidebar--open' : ''}`}>
        ...
        <div className="sidebar__branding">
          <div className="brand-row">
            <BrandLogo
              src="/branding/logo_alcaldia_cbba.png"
              alt="Alcaldía de Cochabamba"
              className="brand-logo brand-logo--alcaldia"
            />
            <div>
              <b>ALCALDÍA DE</b>
          ...
          <div className="brand-row brand-row--dsc">
            <BrandLogo
              src="/branding/logo_dsc.png"
              alt="Dirección de Seguridad Ciudadana"
              className="brand-logo brand-logo--dsc"
            />
            <div>
              <strong>
                SEGURIDAD
        ...
        <div className="sidebar__profile">
          <UserAvatar
            src={user?.photoUrl || guard?.photoUrl}
            name={displayName}
            size={62}
          />
        ...
        <div className="sidebar__footer" aria-hidden="true">
          <div className="sidebar__cityline">
            <span />
            <span />
            <span />
            <span />
